...
bool DetectRegions::verifyLettersInside(Mat image, int qtty) {
    return true;
}
...
bool DetectRegions::verifySizes(RotatedRect mr){
    ...
    int min= 20*20*aspect; // minimum area
    int max= 200*200*aspect; // maximum area
    ...
    if(( area < min || area > max ) || ( r < rmin || r > rmax )
         || (mr.angle < -35 || mr.angle > 35) || (mr.size.width < mr.size.height) ){
        return false;
    }else{
        return true;
    }
...
vector<Plate> DetectRegions::segment(Mat input){
  ...
  if(showSteps)
    ...
    for(int i=0; i< rects.size(); i++){
        ...
        circle(contourImg, rects[i].center, 3, Scalar(0,255,0), -1);
        ...
        //  if(showSteps)
            ...
            if(verifyLettersInside(grayResult, 5)) {
                output.push_back(Plate(grayResult,minRect.boundingRect()));
            }
//            imshow("Last step", grayResult);
        }
    }
